...
<div id="content">
    ...
    <div class="box">
        <div class="heading">
	    	...
	    	<div class="buttons"><a onclick="doBackOrder();" class="button"><?php echo $button_save; ?></a></div>
        </div>
        <div class="content">
			...
			<div id="tab-product" class="vtabs-content">
				<div id="backorder_info" style="">
					<form method="POST" action="<?php echo $link_save; ?>" id="form_backorder" >
						<input type="hidden" name="proNummers" id="orderNummers" />
						<?php echo $text_message_customer; ?><br />
						<textarea name="customer_info" id="customer_info" ><p>Geachte klant,</p><p>Helaas hebben wij niet alle producten uit uw bestelling op voorraad. Een deel van uw bestelling is hierdoor in backorder gevallen. We hebben uw order gesplitst. Order nummer&nbsp;{order_id_1} word direct verzonden,&nbsp;{order_id_2} word verzonden zodra de producten weer leverbaar zijn.</p><p>Ga naar uw account om uw orderoverzicht te kunnen zien, zodat u ziet welke producten wel en niet zijn uitgeleverd.</p><p>Met vriendelijke groet,<br /><span style="line-height: 1.6em;">Het team van Verburg Services</span></p></textarea>
						<br />
						<span style="font-size:90%"><?php echo $text_message_help;?> </span>
						<br />
						<br />
						<?php
							$order_statuses1 = $order_statuses;
							$order_statuses2 = $order_statuses1;
						?>
						<?php echo $text_new_order_status_1;?>
						<select name="order_status_1">
							<?php foreach ($order_statuses1 as $order_statuses_1) { ?>
							<?php if ($order_statuses_1['order_status_id'] == $order_status_id) { ?>
								<option value="<?php echo $order_statuses_1['order_status_id']; ?>" selected="selected"><?php echo $order_statuses_1['name']; ?></option>
							<?php } else { ?>
								<option value="<?php echo $order_statuses_1['order_status_id']; ?>"><?php echo $order_statuses_1['name']; ?></option>
							<?php } ?>
						<?php } ?>
						</select>
						<br />
						<?php echo $text_new_order_status_2;?>
						<select name="order_status_2">
							<?php foreach ($order_statuses2 as $order_statuses_2) { ?>
							<?php if ($order_statuses_2['order_status_id'] == $order_status_id) { ?>
								<option value="<?php echo $order_statuses_2['order_status_id']; ?>" selected="selected"><?php echo $order_statuses_2['name']; ?></option>
							<?php } else { ?>
								<option value="<?php echo $order_statuses_2['order_status_id']; ?>"><?php echo $order_statuses_2['name']; ?></option>
							<?php } ?>
						<?php } ?>
						</select>
						<br /><br /><br />
				...
				<table class="list">
					...
					<tbody>
						<?php foreach ($products as $product) { ?>
							<tr>
								<td><input type="checkbox" name="orderSelect" value="<?php echo $product['product_id']; ?> " /></td>
...
<script>
	var saveLink = "<?php echo html_entity_decode($link_save); ?>";
	
	function doBackOrder(){
		var answer = confirm("Weet u zeker dat u met de ingevoerde gegevens een backorder wilt maken?");
		if (!answer){
			return false;
		}
        ...
        $("input:checkbox[name=orderSelect]:checked").each(function(){
            ...
            orNumList += $(this).val().trim();
        });
        orNumList = orNumList.trim();
        
        // $("#backorder_info").css("display", "inherit");
        $("#orderNummers").val(orNumList);
