...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
		...
		$sql = "SELECT p.product_id, p.image, pd.name, p.model, p.price, p.quantity, p.status FROM " . DB_PREFIX . "product p ";
		...
		if (!empty($data['filter_model'])) {
			...
			$sql .= " p.model LIKE '" . $this->db->escape($data['filter_model']) . "%'";
		}
		...
		if (!empty($data['filter_price'])) {
			...
			$sql .= " p.price LIKE '" . $this->db->escape($data['filter_price']) . "%'";
		}
		...
		if (isset($data['filter_quantity']) && !is_null($data['filter_quantity'])) {
			...
			$sql .= " p.quantity = '" . $this->db->escape($data['filter_quantity']) . "'";
		}
		...
		if (isset($data['start']) || isset($data['limit'])) {
			...
			$sql .= " LIMIT " . (int)$data['start'] . "," . ((int)$data['start'] + 100);
		}	
