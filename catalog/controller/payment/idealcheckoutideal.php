...
class ControllerPaymentIdealcheckoutideal extends Controller
{
	public function index() 
	{
		...
		$this->data['continue'] = $this->url->link('payment/idealcheckoutideal/setup');

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/idealcheckoutideal.tpl')) 
		{
			$this->template = $this->config->get('config_template') . '/template/payment/idealcheckoutideal.tpl';
		}
		else
		{
			$this->template = 'default/template/payment/idealcheckoutideal.tpl';
		}
	...
	{		
		...
		if(isset($this->session->data['order_id']))
		{
			...
			$this->language->load('payment/idealcheckoutideal');
			...
			$sTransactionSuccessUrl = $this->url->link('payment/idealcheckoutideal/callback'); // HTTPS_SERVER . 'index.php?route=payment/idealcheckoutideal/callback';
	...
	public function callback() 
	{
		...
		if($order_info)
		{
			...
			if(isset($oRecordSet->rows[0]) && ($oRecord = $oRecordSet->rows[0]))
			{
				...
				{
					$this->model_checkout_order->update($this->session->data['order_id'], 1, '', false);
