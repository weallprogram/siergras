...
<?php echo $header; ?>
<style type="text/css">
	#edit-c, #edit-m, #edit-p, .down-triangle, .hidden {
		display: none;
	}
...
<div class="box">
	...
	<div class="content">
		<form action="" method="post" enctype="multipart/form-data" id="form">
			...
			<div class="bluebar">
				<select name="edit" onchange="$(this).find('option[value=0]').remove(); $('#edit-c').slideUp(); $('#edit-m').slideUp(); $('#edit-p').slideUp(); if ($(this).val()) $('#' + $(this).attr('name') + '-' + $(this).val().substring(0,1)).slideDown();">
			...
			<div id="edit-p">
				<span style="display: inline-block">
					<select id="editable-list" onchange="getProducts('#editable-list', '#editing-list')">
					...
					<br />
					<select id="editing-list" multiple="multiple"></select>
				</span>
				...
				<span style="display: inline-block">
					...
					<br />
					<select id="edited-list" multiple="multiple"></select>
				</span>
			</div>
			...
			<div class="hidden">
				...
				<tr <?php if ($version < 150) echo 'style="display: none"'; ?>>
					<td><?php echo (isset($entry_upc)) ? $entry_upc: ''; ?></td>
					<td><input type="text" name="upc" /></td>
				</tr>
				<tr <?php if ($version < 154) echo 'style="display: none"'; ?>>
				...
				<tr>
					...
					<td><div class="image">
							...
							<a onclick="image_upload('image', 'thumb');"><img src="<?php echo $this->model_tool_image->resize('no_image.jpg', 100, 100); ?>" alt="" id="thumb" /></a>
							<input type="hidden" name="image" id="image" />
							<br />
							<a onclick="image_upload('image', 'thumb');"><?php echo $text_browse; ?></a>
							&nbsp; | &nbsp;
							<a onclick="$('#thumb').attr('src', '<?php echo $this->model_tool_image->resize('no_image.jpg', 100, 100); ?>'); $('#image').attr('value', '');"><?php echo $text_clear; ?></a>
						</div>
					</td>
				</tr>
				<tr>
					<td><?php echo $entry_date_available; ?></td>
					<td><input type="text" name="date_available" size="12" class="date" /></td>
				</tr>
				<tr>
				...
				<tr  <?php if ($version < 150) echo 'style="display: none"'; ?>>
					...
					<td><table class="list" style="width: 500px; margin-bottom: 0">
							<thead>
								<tr>
				...
				<table class="form">
					...
					<tr>
						...
						<td><span style="display: inline-block">
								<select id="relatable-list" onchange="getProducts('#relatable-list', '#relating-list')">
									<option value="-1"><?php echo $text_select_product; ?></option>
									<optgroup label="<?php echo $text_all_products; ?>">
										<option value="0"><?php echo $text_all_products; ?></option>
									</optgroup>
								...
								<br />
								<select id="relating-list" multiple="multiple"></select>
							</span>
							<span style="display: inline-block; vertical-align: top">
								<br /><br /><br /><br />
							...
							<span style="display: inline-block">
								...
								<br />
								<select id="related-list" multiple="multiple"></select>
							</span>
			...
			<div class="hidden">
				...
				<table class="list">
					...
					<tfoot>
						<tr>
							<td colspan="7" class="left"><a onclick="addRow($(this), 'discount')" class="button"><span><?php echo $button_add_discount; ?></span></a></td>
						</tr>
					</tfoot>
				</table>
			</div>
			...
			<div class="hidden">
				...
				<table class="list">
					...
					<tfoot>
						<tr>
							<td colspan="6" class="left"><a onclick="addRow($(this), 'special')" class="button"><span><?php echo $button_add_special; ?></span></a></td>
						</tr>
					</tfoot>
				</table>
			</div>
...
<script type="text/javascript"><!--
	...
	function getProducts(dropdown, productlist) {
		...
		$.ajax({
			url: 'index.php?route=<?php echo $type; ?>/<?php echo $name; ?>/getProducts&id=' + $(dropdown).val() + '&token=<?php echo $token; ?>',
			dataType: 'json',
	...
	function addRow(element, type) {
		element.parent().parent().parent().prev().append('\
			<tr>\
			<td class="left">\
				<select name="product_' + type + '[customer_group_id][]">\
					<?php foreach ($customer_groups as $customer_group) { ?>\
			...
			<td class="left"><input type="text" name="product_' + type + '[date_start][]" class="date" /></td>\
			<td class="left"><input type="text" name="product_' + type + '[date_end][]" class="date" /></td>\
			<td class="left"><a class="button" onclick="$(this).parent().parent().remove()"><span><?php echo $button_remove; ?></span></a></td>\
			</tr>\
		');
		$('.date').datepicker({dateFormat: 'yy-mm-dd'});
	}
